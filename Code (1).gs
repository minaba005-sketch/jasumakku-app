/**
 * じゃちゅ支払 & アベニール 管理アプリ用 Web API (JSONP対応版)
 *
 * 使い方:
 * 1. このコードをスプレッドシートの「拡張機能 > Apps Script」に貼り付けて保存
 *    (既存のCode.gsの内容を全部削除してから貼り付けてください)
 * 2. 関数選択で setupInitialData を選び、一度実行(初回のみ。既存データを書き込みます)
 *    -> 実行時に権限承認が求められるので許可する
 *    -> すでに実行済みの場合はスキップしてOK(再実行するとデータが初期状態に戻ります)
 * 3. 「デプロイ > デプロイを管理」→ 既存デプロイの編集(鉛筆) → 新バージョン → デプロイ
 *    (アクセスできるユーザー: 全員 のまま)
 */
...
// ============ Web API (すべてGET, JSONP対応) ============
//
// doGet のクエリパラメータ:
//   action=getAll                              -> 全データ取得
//   action=saveJachu&data=<JSON文字列>          -> じゃちゅ支払を保存
//   action=saveAvenir&data=<JSON文字列>         -> アベニールを保存
//   callback=<関数名>                           -> JSONPとして返す(省略時は素のJSON)
...
function doGet(e) {
  ...
  } else if (action === 'saveJachu') {
    const data = JSON.parse(e.parameter.data);
    saveJachuData(data);
    result = { ok: true };
  } else if (action === 'saveAvenir') {
    const data = JSON.parse(e.parameter.data);
    saveAvenirData(data);
    result = { ok: true };
  } else {
